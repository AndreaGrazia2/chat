...
// Imports from views-month.js
import {
    renderizzaVistaMensile
} from './views-month.js';

// Imports from views-week.js
import {
...
export function aggiornaVista() {
    console.log('Aggiornamento vista:', window.vistaAttuale);
    ...
    document.getElementById(`${window.vistaAttuale}View`).classList.add('active');
    ...
    switch (window.vistaAttuale) {
        ...
        case 'week':
            ...
            break;
        case 'day':
            // NUOVO: Se siamo nella vista giornaliera, assicuriamoci che dataSelezionata sia sincronizzata con window.dataAttuale
            if (!dataSelezionata || !isStessoGiorno(dataSelezionata, window.dataAttuale)) {
                dataSelezionata = createDate(window.dataAttuale);
                console.log('Vista giornaliera: dataSelezionata sincronizzata con window.dataAttuale', dataSelezionata);
            }
            renderizzaVistaGiornaliera();
            break;
        case 'list':
    ...
    setTimeout(() => {
        if (window.dragDrop && typeof window.dragDrop.init === 'function') {
            window.dragDrop.init(window.vistaAttuale);
        }
...
export function aggiornaIntestazione() {
    ...
    switch (window.vistaAttuale) {
        case 'month':
            ...
            currentDateElement.textContent = `${mesi[window.dataAttuale.getMonth()]} ${window.dataAttuale.getFullYear()}`;
            break;
        case 'week':
            ...
            break;
        case 'day':
            currentDateElement.textContent = formatDateItalian(window.dataAttuale);
            break;
        case 'list':
            currentDateElement.textContent = formatDateItalian(window.dataAttuale, false);
            break;
    }
}
...
export function renderizzaMiniCalendario() {
    ...
    const anno = window.dataAttuale.getFullYear();
    const mese = window.dataAttuale.getMonth();
    ...
    for (let i = 1; i <= giorniTotali; i++) {
        ...
        const isSelected = window.dataAttuale.getDate() === i && window.dataAttuale.getMonth() === mese && window.dataAttuale.getFullYear() === anno;
    ...
    document.querySelectorAll('.mini-calendar-day').forEach(day => {
        day.addEventListener('click', () => {
            ...
            window.dataAttuale = createDate({ anno, mese, giorno });
            renderizzaMiniCalendario();
            aggiornaVista();
        });
    });
    ...
    eventiGiorno.forEach(evento => {
        html += `
            <div class="events-list-item" data-id="${evento.id}">
                <div class="events-list-item-category" style="background-color: ${window.categorie[evento.categoria].colore};"></div>
