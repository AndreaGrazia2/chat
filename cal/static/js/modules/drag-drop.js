...
export function handleDrop(e) {
    ...
    const eventoOriginale = window.eventi.find(e => e.id === eventId);
    if (!eventoOriginale) {
    ...
    if (window.vistaAttuale === 'day') {
        ...
        const nuovaData = createDate(window.dataAttuale); // Usa window.dataAttuale corrente per giorno/mese/anno
    ...
    else if (window.vistaAttuale === 'week') {
        const ora = parseInt(this.dataset.ora, 10);
    ...
    // Se nella vista mensile:
    else if (window.vistaAttuale === 'month') {
        ...
        // Trova l'evento originale
        const evento = window.eventi.find(e => e.id === eventId);
...
export function enableDragAndDrop() {
    ...
    initDragAndDrop(window.vistaAttuale || 'month');
}
