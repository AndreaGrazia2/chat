...
def register_handlers(socketio):
    ...
    def handle_disconnect(reason=None):  # Modificato per accettare l'argomento reason
    ...
    def handle_channel_message(data):
        ...
        if 'replyTo' in message_data and message_data['replyTo']:
            reply_to_id = message_data['replyTo']['id']
        ...
        with get_db_cursor(commit=True) as cursor:
            cursor.execute(
                ...
                (
                    ...
                    json.dumps(message_data.get('fileData')) if message_data.get('fileData') else None,
    ...
    def handle_direct_message(data):
        ...
        with get_db_cursor() as cursor:
            cursor.execute("SELECT id, username, display_name, avatar_url, status FROM chat_schema.users WHERE id = %s", (user_id,))
            target_user = cursor.fetchone()
            
            cursor.execute("SELECT id, username, display_name, avatar_url, status FROM chat_schema.users WHERE id = 1")
            current_user = cursor.fetchone()
        ...
        message_dict = {
            ...
            'user': {
                ...
                'displayName': current_user['display_name'],  # Rimosso il fallback
